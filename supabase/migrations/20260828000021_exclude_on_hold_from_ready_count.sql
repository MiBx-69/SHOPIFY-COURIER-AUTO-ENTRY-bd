DROP FUNCTION IF EXISTS get_order_counts(uuid);
...
BEGIN
  IF NOT private.has_permission(p_shop_id, 'view_orders') THEN
    RAISE EXCEPTION 'Unauthorized to view orders for this shop' USING ERRCODE = '42501';
  END IF;
  ...
  SELECT 
    ...
    COUNT(*) FILTER (
      ...
      AND fulfillment_status NOT IN ('FULFILLED', 'ON_HOLD')
    ...
    COUNT(*) FILTER (WHERE cancelled_at IS NULL AND fulfillment_status = 'IN_PROGRESS') as preparing_count
  FROM orders
