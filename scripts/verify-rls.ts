...
async function main() {
  ...
  // We will check RLS after reading all migrations
  let fullSql = "";
  ...
  const tenantTables = ["profiles","organizations","memberships","shops","orders","order_line_items","order_events","courier_configs","dispatches","dispatch_attempts","courier_shipments","courier_tracking_events","webhook_events","sync_jobs","sync_errors","audit_logs","saved_filters","order_internal_notes","security_events","shopify_installations","courier_secrets","courier_pickup_preferences"];
  const missing = tenantTables.filter((table) => !fullSql.toLowerCase().includes(`alter table public.${table} enable row level security`));
  if (missing.length) throw new Error(`RLS missing for: ${missing.join(", ")}`);
  
  if (fullSql.toLowerCase().includes("grant all on all tables")) throw new Error("Unsafe database privilege detected");
