...
export function verifyShopifyHmac(params: URLSearchParams) {
  ...
  const digest = crypto.createHmac("sha256", serverEnv().SHOPIFY_CLIENT_SECRET).update(message).digest("hex");
...
export function verifyWebhook(rawBody: string, hmac: string | null) {
  ...
  const digest = crypto.createHmac("sha256", serverEnv().SHOPIFY_CLIENT_SECRET).update(rawBody, "utf8").digest("base64");
