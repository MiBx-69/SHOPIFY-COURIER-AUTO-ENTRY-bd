...
export class DispatchService {
  async execute(dispatchId: string, requestedConfigId?: string) {
    ...
    if (result.outcome !== "success") {
      ...
      if (result.outcome === "unknown") {
        return admin.from("dispatches").update({ phase: "unknown", safe_error_message: "Courier outcome is unknown; reconcile before retrying." }).eq("id", dispatch.id).select().single();
      }
  ...
  async cancel(orderIdOrDispatchId: string, actorId?: string, reason?: string) {
    ...
    if (!dispatch) {
      ...
      await admin.from("orders").update({ dispatch_status: "cancelled" }).eq("id", order.id);
    ...
    if (trackingId && courierConfig?.couriers?.provider) {
      ...
      if (shipment?.id) {
        await admin.from("courier_shipments").update({ status: "cancelled" }).eq("id", shipment.id as string);
    ...
    await admin.from("orders").update({ dispatch_status: "cancelled" }).eq("id", dispatch.order_id);
  ...
  private async fail(dispatch: { id: string; order_id: string }, message: string) {
    const admin = createAdminClient();
    await admin.from("dispatches").update({ status: "failed", phase: "failed", safe_error_message: message }).eq("id", dispatch.id);
    await admin.from("orders").update({ dispatch_status: "failed" }).eq("id", dispatch.order_id);
    return admin.from("dispatches").select().eq("id", dispatch.id).single();
  }
}
