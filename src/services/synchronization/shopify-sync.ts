...
export class ShopifySyncService {
  ...
  private async upsertOrder(shopId: string, source: ShopifyOrder) {
    const admin = createAdminClient();
    const shippingNodes = source.shippingLines?.nodes || [];
    const primaryShippingTitle = shippingNodes[0]?.title || null;
    ...
    const row = {
      ...
      shipping_lines: shippingNodes,
      shipping_title: primaryShippingTitle,
      note: source.note,
      ...
      fulfillment_status: source.displayFulfillmentStatus === "UNFULFILLED" && source.fulfillmentOrders?.nodes.some(fo => fo.status === "IN_PROGRESS") ? "IN_PROGRESS" : source.displayFulfillmentStatus,
